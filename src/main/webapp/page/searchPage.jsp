...
<%
		...
		if (startPage > 1) {%>
			<a href="<%=gender%>?page=1&keyword=<%=keyword%>">처음</a>
		<% }
		
		if (pg > 1) {%>
			<a href="<%=gender%>?page=<%=pg-1%>&keyword=<%=keyword%>">이전</a>
		<% }
		...
		for (int iCount = startPage; iCount <= endPage; iCount++) {
			...
			<% } else { %>
				...
				&nbsp;<a href="<%=gender%>?page=<%=iCount%>&keyword=<%=keyword%>"><%=iCount %></a>&nbsp;
			
			<% }
		}
		
		if (pg < totalPage) { %>
			<a href="<%=gender%>?page=<%=pg+1%>&keyword=<%=keyword%>">다음</a>
		<% }

		if (endPage < totalPage) { %>
			<a href="<%=gender%>?page=<%=totalPage%>&keyword=<%=keyword%>">끝</a>
		<% }
