...
(function(window){
    ...
    function lignePaginate(){
        ...
        var pageButtons = function(numberOfPage,currentPage) {
             ...
             **/
            let buttons = "<input type='button' value='이전' class='btn btn-primary' onclick='paginate.sort("+(currentPage - 1)+")' "+prevDisabled+">";
            let buttonNumberOfPage = "<input type='button' value='" + currentPage + ' - ' + numberOfPage + "' disabled class = 'btn btn-outline-primary'>";

            for (let $i=1; $i<=numberOfPage;$i++){
                if(numberOfPage > 10){
                    buttons += paginationMoreThatTenPage($i,numberOfPage);
                }else{
                    buttons += "<input type='button' id='id"+$i+"'value='"+$i+"' onclick='paginate.sort("+$i+")' class='btn btn-outline-secondary'>";
                }
            }

            let nextButton = "<input type='button' value='다음' class='btn btn-primary' onclick='paginate.sort("+(currentPage + 1)+")' "+nextDisabled+">";
            buttons +=  nextButton;
        ...
        var addGoToPage = function(){
            let inputBox = "<input type='number' id='paginate_page_to_go' value='1' min='1' max='"+ settings.numberOfPages +"' class='btn btn-outline-secondary'>";
            let goButton = "<input type='button' id='paginate-go-button' value='이동' onclick='paginate.goToPage()' class = 'btn btn-primary'>  ";
